...
def split_features_labels(df):
    ...
    # 检查标签值是否在 [0, 1] 范围内
    assert np.all((y >= 0) & (y <= 1)), "标签值必须在 [0, 1] 范围内"
    return X, y
...
class TransformerMoERecommender(nn.Module):
    ...
    def forward(self, x):
        x = self.embedding(x)
        # 添加序列维度，让 x 的形状变为 (batch_size, 1, d_model)
        x = x.unsqueeze(1)
        x = self.positional_encoding(x)
        x = self.transformer_encoder(x)
        x = x.squeeze(1)  # 移除序列维度 [batch_size, 1, feature_dim] -> [batch_size, feature_dim]
        x = self.moe(x)
...
def train(model, dataloader, optimizer, criterion):
    ...
    for X, y in dataloader:
        ...
        optimizer.zero_grad()
        outputs = model(X)
        # 裁剪输出值到 [0, 1] 范围，确保数值稳定性
        outputs = torch.clamp(outputs, 0, 1)
        loss = criterion(outputs, y)
        print(f"train index {index} total_loss {total_loss} outputs: {outputs}")
        print(f"train index {index} total_loss {total_loss} y: {y}")
        loss.backward()
        optimizer.step()
        lossItem = loss.item()
        print(f"train index {index} total_loss {total_loss} loss: {lossItem:.4f}")
...
def evaluate(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            total_loss += loss.item()
            index += 1
            print(f"evaluate cuda index {index} total_loss {total_loss} loss: {loss.item():.4f}")
            if index == 7:
                break
